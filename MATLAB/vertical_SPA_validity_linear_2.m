...
A0 = 1./rP0.^2./rG0;
Phi0 = -k*(F*rP0+rG0);
ddPhi0 = -k*(F./rP0 + 1./rG0);
ddddPhi0 = 3*k*(F./rP0.^3 + 1./rG0.^3);
ddA0 = -(2*rG0^2+rP0.^2)./(rG0.^3.*rP0.^4);
I0 = sqrt(2*pi)*exp(1i*pi/4*sign(ddPhi0))./abs(ddPhi0).^(0.5).*A0.'.*exp(1i*Phi0);

I2 = sqrt(2*pi)/24*exp(3*1i*pi/4*sign(ddPhi0))./(ddPhi0.^2.*abs(ddPhi0).^(3/2)).*exp(1i*Phi0).*...
    (ddA0.*ddPhi0.^2 +A0/12.*(-3*ddddPhi0.*ddPhi0));
...
dBdiff = abs(20*log10(abs(I))-20*log10(abs(I0)));
ixs = find(dBdiff < 3);
...
hold on
semilogx(freq, 20*log10(abs(I0.*sqrt(omega))),'LineWidth',1.25);
grid on
...
print( '-r300','integral_comparison_setup3' ,'-dpng')
%%
xl = [-1.5, 1.5];
yl = [-1.2, 1.2];
