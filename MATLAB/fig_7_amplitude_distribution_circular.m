...
for ki = 1 : length(freq)
    k0 = 2*pi*freq(ki)/c;
    Px0 = 1/(4*pi)*exp(-1i*F*k0*rhoP)./rhoP;
    Dx = sqrt(F*8*pi*1i*k0).*(win.*dref.*khn.*Px0*dl);
    Psynth{ki} = reshape( sum(Dx.'.*exp(-1i*k0*R)./R/4/pi,2) ,length(y),length(x));
    R0 = sqrt( (X-xSource(1)).^2 + (Y-xSource(2)).^2  );
    Pref{ki} = 1/(4*pi)*exp(-1i*k0*R0)./R0;
...
unitIxs1 = find(win==1);
cricticalIxs = [ unitIxs1(1), unitIxs1(end)];
D = 10;
...
xl = gca().XLim;
yl = gca().YLim;
for n = 1 : length(cricticalIxs)
    line( [xSource(1) , xSource(1)+D*kh(cricticalIxs(n),1)],...
        [xSource(2) , xSource(2)+D*kh(cricticalIxs(n),2)], ...
...
print( '-r300', 'circular_WFS_setup' ,'-dpng')
...
set(gcf,'PaperPositionMode','auto');
print( '-r300', 'circular_WFS_dA_est' ,'-dpng')

%%
for n = 1 : length(Psynth)
    pos = [ 0.14 0.2 .8 .8];
    ftsize = 13;
    fig = figure('Units','points','Position',[150,150,250,200]);
    p3 = axes('Units','normalized','Position',pos(1,:));

    pcolor(x,y,abs(20*log10(abs(Psynth{n}./Pref{n}))))
    shading interp
    axis equal tight
    clim([0,3])
    c = colorbar;
    ...
    xl = gca().XLim;
    yl = gca().YLim;
    for m = 1 : length(cricticalIxs)
        line( [xSource(1) , xSource(1)+D*kh(cricticalIxs(m),1)],...
            [xSource(2) , xSource(2)+D*kh(cricticalIxs(m),2)], ...
            'Color', 'black', 'LineWidth', LW, 'LineStyle', '--');
    end
    xlim(xl);
    ylim(yl);
    ...
    q= 10;
    draw_ssd( fig , x0(1:q:end,:) + n0(1:q:end,:)*0.02 , n0(1:q:end,:), 0.035 );
    ...
    set(gcf,'PaperPositionMode','auto');
    print( '-r300', sprintf('circular_WFS_dA_mes_%i',n) ,'-dpng')
end
...
% figure
% n = 2;
%  pcolor(x,y,20*log10(abs(Pout{n}-A0.*Pref{n})/P0))
%     shading interp
%     axis equal tight
%     clim([-50,30])
%     c = colorbar;
